...
{
    public class WhenEmailFailed : EmailQueueProjectionSpecification<Projections.Emails.EmailQueueProjection>
    {
        ...
        protected override IEnumerable<IEvent> Given()
        {
            ...
            yield return new EmailFailed(_aggregateId, 2, "");
        }
        ...
        {
            var emailQueue = Context.Find<EmailQueue>(_aggregateId);
            emailQueue.Status.Should().Be(EmailStatus.Failed);
        }
        ...
        {
            var emailQueue = Context.Find<EmailQueue>(_aggregateId);
            emailQueue.Attempts.Should().Be(1);
        }
        ...
        {
            var emailQueue = Context.Find<EmailQueue>(_aggregateId);
            emailQueue.Version.Should().Be(2);
        }
    }
}
